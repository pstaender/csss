    ...
    for (i = _i = 0, _len = lines.length; _i < _len; i = ++_i) {
      ...
      if (this.pattern.isCSSQuery.test(line)) {
        ...
        if ((this.doesLineBeginsWithAttribute(lineBefore) && nextLine && this._indentSpacesOfLine(nextLine) === indentSpacesCount) || isInListedValues) {
          ...
          isInListedValues = (/^\s+[\#a-zA-Z\_\-0-9\(\)\.]+/.test(line) || /^\s+\@/.test(line)) && this.doesLineHaveOnlyAttribute(lineBefore);
        ...
        if (typeof nextLine !== 'string' || (this._indentSpacesOfLine(nextLine) < indentSpacesCount)) {
          line = whitespaces + this.operateInline(line, {
            escape: false,
            enclose: !/^\s*@[a-z\_\-0-9]+(\(.*?\))*\s*$/.test(line),
            withUnit: true
