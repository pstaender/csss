...
DocumentStyle = (function() {
  ...
  DocumentStyle.prototype.page = function(selector, values) {
    ...
    _ref = this._selectorAndValues(selector, values), selector = _ref.selector, values = _ref.values;
    if (selector) {
      return this.addLine("@page " + selector + " " + (this.object_to_css(values)));
    }
  };

  DocumentStyle.prototype._selectorAndValues = function(selector, values) {
...
CSSS = (function() {
  ...
  CSSS.prototype.operateInline = function(s, options) {
    ...
    if (s) {
      ...
      if (onlyIfOperatorsExists && !hasOperators) {
        ...
        if (enclose) {
          return "'" + (s.replace(/(\s@[a-zA-Z]+)(\[[a-zA-Z0-9\_]+\])*(\(.*?\))*\s/g, " ' + $1$2 + ' ")) + "'";
        } else {
          return "" + s;
        }
      }
  ...
  CSSS.prototype.processStyleText = function() {
    var changedLine, i, line, lineBefore, lineBeginsWithAttribute, lines, matches, styletext, _i, _len;
    ...
    for (i = _i = 0, _len = lines.length; _i < _len; i = ++_i) {
      ...
      if (lineBefore && lineBeginsWithAttribute && /^\s*(@[a-z\_\-]+)([^\,]\s*)*\s*$/i.test(lineBefore)) {
        matches = lineBefore.match(/^(\s*)(@[a-z\_\-]+)([^a-z\_\,].*)*\s*$/i);
        changedLine = matches[1] + matches[2];
        lines[i - 1] = changedLine += (matches[3] ? matches[3] + ',' : '') + ' __$cssAttributes__ =';
      }
      lines[i] = line;
    }
